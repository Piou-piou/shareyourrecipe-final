<?php
	...
	//recuperation d'un message posté dans l'url
	$mess = $_GET['mess'];
	
	//recuperation de l'id pseudo pour avoir les recettes
	$reqpseudo = "select ID_pseudo from pseudo where pseudo='$pseudo'";
	$exereqpseudo = mysql_query($reqpseudo, $dbc);
	$row = mysql_fetch_row($exereqpseudo);
	$id_pseudo = $row[0];
	
	//recuperation des recettes
	$req = "select * from recette where ID_pseudo='$id_pseudo'";
                    	...
                    	<div class="contenuphpmembre">
							<?php
                                while($ligne = mysql_fetch_assoc($query)) {
                                    ...
                                    $id_pseudo = $ligne['ID_pseudo'];
                                    $nom_recette = $ligne['nom_recette'];
                                    echo("<p><a href='recherche/affiche_recette.php?id_recette=$id_recette'>$nom_recette</a></p><p><a href='modif_recette.php?id_recette=$id_recette'> Modifiez cette recette</a></p>");
                                    echo("<p><a href='supprim_recette.php?id_recette=$id_recette'> Supprimer cette recette</a></p><br>");
                                }
                                echo($mess);
                            ?>
    
                            <?php echo($pseudo_modif); ?>
