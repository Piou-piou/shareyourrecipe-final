<?php
	...
	//recup du champ de la recherche
	$nom = mysql_real_escape_string(htmlspecialchars($_POST['recherche']));
	
	$req = "select * from recette where nom_recette like '%$nom%' order by nom_recette DESC";
					...
					<div class="colGauche">
                    	<div class="contenuphpmembre">
							...
							<?php
                                ...
                                echo("<h2>Cliquez sur la recette que vous souhaitez consulter.</h2>");
                                while($ligne = mysql_fetch_assoc($query)) {
                                    $id_recette = $ligne['ID_recette'];
                                    $id_pseudo = $ligne['ID_pseudo'];
                                    $nom_recette = $ligne['nom_recette'];
                                    echo("<p><a href='affiche_recette.php?id_recette=$id_recette'>$nom_recette</a></p>");
                                }
                            ?>   
                    	</div>
                    </div> <!--FIN COLGAUCHE-->
