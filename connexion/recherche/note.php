	...
	if (isset($_POST['submittednote'])) {
		...
		//sinon on insere la note et on calcul la moyenne
			...
			if ($note > 5) {
				...
				//verif si id_recette deja ok dans moyenne
				$id_recettemoyenne = "select * from moyenne where ID_recette='$id_recette'";
				$exeid_recettemoyenne = mysql_query($id_recettemoyenne, $dbc);
				$numid = mysql_num_rows($exeid_recettemoyenne);
				if ($numid == 0) {
					//insertion de la moyenne dans la bdd
					$req3 = "insert into moyenne (ID_recette, moyenne) values ('$id_recette', '$moy')";
